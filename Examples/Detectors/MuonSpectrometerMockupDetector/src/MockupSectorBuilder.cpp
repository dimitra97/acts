...
ActsExamples::MockupSectorBuilder::buildChamber(
  ...
  for (auto& detectorElement : detectorElements) {
    ...
    // detectorElement->transform(gctx).pretranslate(Acts::Vector3(center.x(),
    // center.y(), center.z() + mCfg.zOffset));
...
ActsExamples::MockupSectorBuilder::buildSector(
  ...
  for (int i = 0; i < mCfg.NumberOfSectors; i++) {
    ...
    for (int itr = 0; itr < detVolumesSize; itr++) {
      ...
      detectorVolumeSec->assignGeometryId(
          Acts::GeometryIdentifier{}.setLayer(itr + 1).setVolume(
  ...
  for (std::size_t i = 0; i < cylinderVolumesBounds.size(); ++i) {
    detectorCylinderVolumesOfSector.push_back(
        Acts::Experimental::DetectorVolumeFactory::construct(
            Acts::Experimental::defaultPortalAndSubPortalGenerator(), gctx,
            "cylinder_volume_layer_" + std::to_string(i), transform,
  ...
  detectorVolume->assignGeometryId(Acts::GeometryIdentifier{}.setVolume(1500));

  return detectorVolume;
}

std::shared_ptr<Acts::Experimental::Detector> ActsExamples::MockupSectorBuilder::buildMuonDetector(
  std::vector<std::shared_ptr<Acts::Experimental::DetectorVolume>> rootVolumes){

   return Acts::Experimental::Detector::makeShared(mCfg.detectorName, std::move(rootVolumes), 
...
ActsExamples::MockupSectorBuilder::buildMultiLayer(
  ...
  std::cout << rotX(0) << ", " << rotX(1) << ", " << rotX(2) << std::endl;
  std::cout << rotY(0) << ", " << rotY(1) << ", " << rotY(2) << std::endl;
  std::cout << rotZ(0) << ", " << rotZ(1) << ", " << rotZ(2) << std::endl;
  // std::cin.ignore();
